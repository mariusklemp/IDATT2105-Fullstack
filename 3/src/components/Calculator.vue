...
export default {
  ...
  methods: {
    ...
    divide() {
      this.operatorSign = "/"
      this.operator = (a, b) => (b / a).toFixed(3);
      this.previous = this.screen;
      this.operatorClicked = true;
    },
    times() {
      this.operatorSign = "*"
      this.operator = (a, b) => (a * b).toFixed(3);
      this.previous = this.screen;
      this.operatorClicked = true;
    },
    equal() {
      let answer = `${this.operator(
          parseFloat(this.screen),
          parseFloat(this.previous)
      )}`;
      this.logArray.push(this.previous + this.operatorSign + this.screen + "= " + answer);
      this.screen = answer;
      this.previous = "";
    },
    clearLog(){
